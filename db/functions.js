...
function readAllDepartments() {
  const sql =
    "SELECT dpt_name AS department, id FROM departments ORDER BY department";
  const params = [];
  return connection
    .promise()
    .query(sql, params)
...
function readAllManagers() {
  const sql = `SELECT DISTINCT emp1.manager_id, CONCAT(emp2.first_name, ' ', emp2.last_name) as mgr_name
        FROM employees emp1
        INNER JOIN employees emp2 ON emp1.manager_id = emp2.id
        ORDER BY mgr_name`;
  const params = [];
  return connection
    .promise()
